...
def generate_graphs(stats, graphs_dir):
    ...
    if scaling_summary and scaling_summary.exists():
        ...
        if seq_by_threads:
            ...
            all_threads = sorted(seq_by_threads.keys())
            n_max = max(all_threads)
            n_rng = np.linspace(1, n_max * 1.1, 100)
            ax.plot(n_rng, n_rng, 'k--', linewidth=1.5, alpha=0.4, label='Ideal S = N')

            # Sequential como línea de referencia (siempre speedup = 1.0)
            ax.axhline(y=1.0, color='gray', linewidth=1, alpha=0.3, linestyle=':')
            seq_speedups = [1.0] * len(all_threads)
            c_seq = colors.get('sequential', '#1f77b4')
            ax.plot(all_threads, seq_speedups, marker='x',
                    color=c_seq, linewidth=2, markersize=8, label='SEQUENTIAL', zorder=5)
            ...
            ax.set_title('Escalabilidad: Speedup VT vs Threads (1–8) — 5 modelos',
                         fontweight='bold')

...
# de speedup con gráficas (incluyendo eficiencia paralela, escalabilidad y
# comparativas por grupo equivalente).
#
# Uso: ./scripts/run_all_models.sh [--skip-smt] [--skip-fgmt] [--skip-cmp] [--with-perf]
#
# Flags opcionales:
#   --skip-fgmt  Omitir FGMT (lento: ~102s con semáforos, ~204s legacy)
#   --skip-smt   Omitir SMT  (lento: varía según implementación)
#   --skip-cmp   Omitir CMP
#   --with-perf  Ejecutar perf stat al final (requiere 'perf' instalado)
#
...
[ "$WITH_PERF" = true ] && echo "  ✓ perf stat habilitado (--with-perf)"
echo ""
...
# ── Perfilado con perf stat (opcional) ───────────────────────────────────────
echo ""
if [ "$WITH_PERF" = true ]; then
    echo "Ejecutando perfilado con perf stat..."
    # Perfilar modelos reales (los que se ejecutaron)
    PERF_MODELS=("sequential")
    [ "$SKIP_CMP"  = false ] && PERF_MODELS+=("cmp")
    [ "$SKIP_SMT"  = false ] && PERF_MODELS+=("smt")
    bash "$SCRIPTS_DIR/run_perf.sh" "${PERF_MODELS[@]}" || \
        echo "    ⚠ run_perf.sh falló (revisar instalación de perf)"
else
    echo "  [ perf omitido — usar --with-perf para habilitar ]"
    echo "  Perfilado manual: ./scripts/run_perf.sh"
fi
...
echo "  ├─ mediciones_cmp.csv"
echo "  ├─ speedup_report.log  (incluye speedup, eficiencia paralela, escalabilidad)"
echo "  ├─ graficas/"
echo "  │  ├─ 01_histogram_comparativo_gmt.png"
echo "  │  ├─ 01_histogram_comparativo_s-cmp.png"
echo "  │  ├─ 02_boxplot_comparativo_gmt.png"
echo "  │  ├─ 02_boxplot_comparativo_s-cmp.png"
echo "  │  ├─ 03_speedup_comparison.png        (VT todos los modelos)"
echo "  │  ├─ 04_timeline_executions.png"
echo "  │  ├─ 05_virtual_time_comparison.png"
echo "  │  ├─ 06_cpu_speedup_smt_cmp.png       (SMT/CMP CPU time)"
echo "  │  ├─ 07_efficiency.png                ← NUEVO: eficiencia paralela E=S/N"
echo "  │  ├─ 08_scalability.png               ← NUEVO: escalabilidad N vs Speedup"
echo "  │  ├─ 09_modeled_group_comparison.png  ← NUEVO: seq vs fgmt vs cgmt (VT)"
echo "  │  └─ 10_real_group_comparison.png     ← NUEVO: seq vs smt vs cmp (CPU)"
echo "  └─ image/"
...
echo "     └─ frame_cmp.ppm"
echo ""
echo "  Scripts de análisis adicionales:"
echo "    ./scripts/run_smt_comparison.sh  # mide CMP con SMT on/off"
echo "    ./scripts/run_perf.sh            # perf stat para todos los modelos"
echo ""
